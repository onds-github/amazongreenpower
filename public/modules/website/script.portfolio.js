...
$(document).ready(function () {
    ...
    var _url = '/website/portfolio/';
    var _id = 'id_portfolio';

    var _fields = {
        name_portfolio: {
            identifier: 'name_portfolio',
            rules: [
                {
                    type: 'empty',
                    prompt: 'Informe'
    ...
    var _table = $('#onTablePortfolio').DataTable({
        ajax: _url + 'ajax',
        language: {
    ...
    $(document).on('click', 'tr td .on-button-files', function () {
        ...
        $('body').onUploadFile({
            ...
            nameTableParent: 'on_portfolio',
            singleFile: true,
            onSuccess: function(response) {
                $.ajax({
                    type: 'POST',
                    url: _url + 'update',
                    data: {q: row[_id], data: 'image_portfolio=' + response.result.link_file},
                    success: function (response) {
                        _table.ajax.reload();
                    }
                });
