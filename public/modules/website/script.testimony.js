...
$(document).ready(function () {
    ...
    var _url = '/website/testimony/';
    var _id = 'id_testimony';

    var _fields = {
        name_testimony: {
            identifier: 'name_testimony',
            rules: [
                {
                    type: 'empty',
                    prompt: 'Informe'
    ...
    var _table = $('#onTableTestimony').DataTable({
        ajax: _url + 'ajax',
        language: {
        ...
        columns: [
            ...
            {
                className: 'on-edit',
                data: 'description_testimony'
            },
            {
                class: 'on-edit',
                data: 'status_testimony',
    ...
    $(document).on('click', 'tr td .on-button-files', function () {
        ...
        $('body').onUploadFile({
            ...
            nameTableParent: 'on_testimony',
            singleFile: true,
            onSuccess: function(response) {
                $.ajax({
                    type: 'POST',
                    url: _url + 'update',
                    data: {q: row[_id], data: 'image_testimony=' + response.result.link_file},
                    success: function (response) {
                        _table.ajax.reload();
                    }
                });
